...
import InfluencerUploadModal from "components/InfluencerUploadModal";
...
const InfluencerPage = ({ creatives, projects, dispatch }) => {
  ...
  const loadProjects = ({ proposalStatus }) => {
    dispatch({
      ...
      payload: {
        query: { proposalStatus: "accepted", include: "creatives" },
        id,
      },
    });
                  ...
                  {creatives.length !== 0 ? (
                    creatives[0]?.creatives.map((creative) => (
                      <Col
                        ...
                        className={`main-creatives-${creative?.status} project-influncer-file-container`}
                      >
                        <DownLoadedFile
                          creative={creative}
