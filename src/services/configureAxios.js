...
const axiosConfigurationForMediaUpload = () =>
  axios.create({
    baseURL: process.env.REACT_APP_API_HOST,
    timeout: 3600 * 1000,
    withCredentials: true,
...
export const axiosInstanceMediaUpload = axiosConfigurationForMediaUpload();

axiosInstanceMediaUpload.interceptors.request.use((config) => {
  const headers = {
    Accept: "application/json",
    withCredentials: true,
  };
...
axiosInstanceMediaUpload.interceptors.response.use(
  (response) => {
    return response.data;
  },
  (err) => {
